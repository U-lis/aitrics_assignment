...
**Context Change:**
Service environment changed to **Server-to-Server** communication. JWT-based authentication is no longer necessary.
Simple Bearer token validation is sufficient for security.
...
**Key Learning:** The actual fix for pytest-asyncio event loop issues is the loop scope configuration, not lazy
initialization of database engines.
